...
- **写作台** — Markdown 编辑、实时预览、手动保存（关闭/崩溃前兜底保存）、版本历史、模板和图片上传。
- **稿件库** — 管理草稿、发布状态和平台版本。
...
Markdown 编辑器支持实时预览、沉浸式全屏模式、手动保存（页面关闭或崩溃前若未保存则触发兜底保存）、版本历史回滚、内容模板和媒体库。
...
参见 [docs/configuration.md](./docs/configuration.md)，包含平台凭证和 AI Agent 设置。
...
基于 pnpm workspaces monorepo 构建：前端是 Vite 8 SPA（`apps/web`，React 19 + react-router），通过 `/api/*` 与独立的 Hono 4 API server（`apps/api`，端口 8787）通信。全栈 TypeScript 5 (strict)、vanilla-extract、Zustand，共享类型放在 `packages/shared-types`。
